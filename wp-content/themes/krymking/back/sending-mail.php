...
function objectPending($post_id) {
    ...
    $messageContent = 'Поздравляем, Вы проделали большую работу и теперь Ваш объект жилья проходит модерацию. ';
    $messageContent .= 'Крымкинг.ру максимально быстро проверит Ваше объявление.';

    $message1 = include $_SERVER['DOCUMENT_ROOT'] . '/wp-content/themes/'. get_template() .'/back/mail_template.php';

    $message1 = str_replace('<<MAILCONTENT>>', $messageContent, $message1);
    $message1 = str_replace('<<FIRSTNAME>>', $author->first_name, $message1);
    $message = str_replace('<<LASTNAME>>', $author->last_name, $message1);
    ...
    $messageContent2 = "Проверьте объект: \n\n";
    $messageContent2 .= '<a href="'.home_url("/wp-admin/post.php?post=".$post_id."&action=edit").'">'.$post_title . ": № " . $post_id .'</a>';

    $message2 = include $_SERVER['DOCUMENT_ROOT'] . '/wp-content/themes/'. get_template() .'/back/mail_template.php';

    $message2 = str_replace('<<MAILCONTENT>>', $messageContent2, $message2);
    $message2 = str_replace('<<FIRSTNAME>>', $author->first_name, $message2);
    $message2full = str_replace('<<LASTNAME>>', $author->last_name, $message2);



    if(!wp_mail( get_option('admin_email'), $subject2, $message2full, $headers)){

        mail( get_option('admin_email'), $subject2, $message2full, $headers);
...
function objectPublish($post_id) {
    ...
    $messageContent = 'Поздравляем, Ваше объявление успешно прошло модерацию и теперь Ваш объект жилья опубликован на Krymking.ru и ему присвоен ID-номер '.$post->ID.' ';
    $messageContent .= 'Редактировать информацию по своим объектам жилья Вы можете в <a href="'.home_url('/profile/objects/').'">Личном кабинете</a>.';
...
function objectRejected($post_id) {
    ...
    $messageContent = 'К сожалению, Ваше объявление не прошло модерацию и Ваш объект жилья не опубликован на Krymking.ru. ';
    $messageContent .= 'Узнать подробности Вы сможете, связавшись с нашей <a href="'.home_url("/about-us/support/").'">Службой поддержки.</a>.';
